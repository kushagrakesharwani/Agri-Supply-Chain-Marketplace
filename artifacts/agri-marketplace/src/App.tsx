...
function ListingDetail({ user }: { user: User }) {
  ...
  const [deliveryWindow, setDeliveryWindow] = useState(deliveryWindowOptions[0]);
  const [message, setMessage] = useState('');
  ...
  const placeOrder = (event: FormEvent) => {
    ...
    setMessage('');
    createOrder.mutate({ data: { buyerId: user.id, listingId: item.id, quantity: Number(quantity), deliveryWindow } }, {
      onSuccess: (order) => {
        ...
        <Card className="h-fit border-border bg-card soft-shadow">
          ...
          <CardContent>
            <div className="flex items-end justify-between border-b border-border pb-5"><div><p className="text-sm text-muted-foreground">Available</p><p className="mt-1 text-xl font-semibold" data-testid={`text-detail-available-${item.id}`}>{item.availableQuantity} {item.unit}</p></div><div className="text-right"><p className="text-sm text-muted-foreground">Price</p><p className="mt-1 font-mono text-2xl text-primary" data-testid={`text-detail-price-${item.id}`}>{money(item.pricePerUnit)}<span className="text-xs text-muted-foreground">/{item.unit}</span></p></div></div>
             {isBuyer ? <form onSubmit={placeOrder} className="mt-6 space-y-5" data-testid="form-place-order"><div><label className="mb-2 block text-sm font-medium" htmlFor="order-quantity">How much do you need?</label><div className="flex items-center gap-2"><Input id="order-quantity" type="number" min="0.01" max={item.availableQuantity} step="0.01" value={quantity} onChange={(event) => setQuantity(event.target.value)} data-testid="input-order-quantity" /><span className="w-16 text-sm text-muted-foreground">{item.unit}</span></div></div><div><label className="mb-2 flex items-center gap-2 text-sm font-medium" htmlFor="order-delivery-window"><CalendarClock className="h-4 w-4 text-primary" /> Pickup / delivery window</label><select id="order-delivery-window" value={deliveryWindow} onChange={(event) => setDeliveryWindow(event.target.value)} className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm" data-testid="select-delivery-window">{deliveryWindowOptions.map((option) => <option key={option} value={option}>{option}</option>)}</select><p className="mt-2 text-xs text-muted-foreground">The farmer will prepare your order for this window.</p></div><div className="flex items-center justify-between rounded-xl bg-muted p-4"><span className="text-sm text-muted-foreground">Order total</span><span className="font-mono text-lg font-medium" data-testid="text-order-total">{money(total)}</span></div>{message ? <p className={`rounded-lg p-3 text-sm ${message.startsWith('Order') ? 'bg-primary/10 text-primary' : 'bg-destructive/10 text-destructive'}`} data-testid="state-order-result">{message}</p> : null}<Button type="submit" className="h-11 w-full" disabled={createOrder.isPending || Number(quantity) <= 0 || Number(quantity) > item.availableQuantity} data-testid="button-place-order">{createOrder.isPending ? <LoaderCircle className="animate-spin" /> : <ShoppingBasket />} Place order</Button></form> : <div className="mt-6 rounded-xl bg-muted p-4 text-sm leading-6 text-muted-foreground" data-testid="state-seller-view">You are viewing this lot as a {roleLabel(user.role)}. Buyer orders will appear in your trade desk.</div>}
          </CardContent>
        </Card>
      </div>
    </div>
    ...
    <div className="mx-auto max-w-6xl px-4 py-8 md:px-10 md:py-12">
      ...
      <div className="mt-9 grid gap-8 lg:grid-cols-[1fr_360px]">
        <section>
         ...
         <aside>
           {selectedOrderId ? <OrderDetail selected={selected.data} isLoading={selected.isLoading} onClose={() => setSelectedOrderId(null)} onAdvance={() => { if (selected.data) advanceOrder(selected.data); }} updating={updateStatus.isPending} /> : isSeller ? <SellerListings listings={listingItems} isLoading={listings.isLoading} editingId={editingId} editPrice={editPrice} onEdit={(listing) => { setEditingId(listing.id); setEditPrice(String(listing.pricePerUnit)); }} onCancel={() => setEditingId(null)} onPriceChange={setEditPrice} onSave={saveListing} onDelete={(listing) => { if (window.confirm(`Remove the ${listing.cropType} listing?`)) deleteListing.mutate({ listingId: listing.id }, { onSuccess: () => { qc.invalidateQueries({ queryKey: getListListingsQueryKey({ sellerId: user.id }) }); setActionMessage('Listing removed from the market.'); } }); }} /> : <div className="line-art rounded-2xl border border-border p-6"><p className="eyebrow text-primary">A clear path</p><h2 className="serif mt-3 text-2xl">No guesswork.</h2><p className="mt-3 text-sm leading-6 text-muted-foreground">Tap an order to see its full record. We keep the next step simple so you can plan around it.</p></div>}
        </aside>
      </div>
    </div>
  );
...
function OrderRow({ order, isSeller, onSelect, onAdvance, updating }: { order: Order; isSeller: boolean; onSelect: () => void; onAdvance: () => void; updating: boolean }) {
  const next = statuses[Math.min(statuses.indexOf(order.status) + 1, statuses.length - 1)];
  const nextActionLabel = order.status === 'ready' ? 'Mark picked up / delivered' : `Mark ${statusLabel(next)}`;
  return <Card className="pressable cursor-pointer border-border bg-card" onClick={onSelect} data-testid={`row-order-${order.id}`}><CardContent className="p-4 md:p-5"><div className="flex items-start justify-between gap-3"><div><p className="mono text-[10px] uppercase tracking-[.12em] text-muted-foreground">Order {order.id.slice(0, 8)}</p><p className="mt-1 font-medium" data-testid={`text-order-id-${order.id}`}>{order.quantity} units · {money(order.totalAmount ?? 0)}</p></div><StatusBadge status={order.status} /></div><div className="mt-5 flex items-center gap-1">{statuses.map((status, index) => <div key={status} className="flex flex-1 items-center gap-1"><span className={`h-2 w-2 shrink-0 rounded-full ${index <= statuses.indexOf(order.status) ? 'bg-primary' : 'bg-muted'}`} /><div className={`h-1 flex-1 rounded-full ${index < statuses.indexOf(order.status) ? 'bg-primary' : 'bg-muted'}`} /></div>)}</div><div className="mt-3 flex flex-col gap-2 text-xs text-muted-foreground"><span className="flex items-center gap-1"><CalendarClock className="h-3.5 w-3.5 text-primary" /> {order.deliveryWindow ?? 'Window pending'}</span><div className="flex items-center justify-between gap-3"><span>{dateLabel(order.createdAt)}</span>{isSeller && next !== order.status ? <Button type="button" variant="outline" size="sm" onClick={(event) => { event.stopPropagation(); onAdvance(); }} disabled={updating} data-testid={`button-advance-order-${order.id}`}>{updating ? <LoaderCircle className="animate-spin" /> : null} {nextActionLabel}</Button> : <span className="flex items-center gap-1">View details <ArrowRight className="h-3 w-3" /></span>}</div></div></CardContent></Card>;
}

function OrderDetail({ selected, isLoading, onClose, onAdvance, updating }: { selected?: Order; isLoading: boolean; onClose: () => void; onAdvance: () => void; updating: boolean }) {
  if (isLoading) return <div className="skeleton h-56 rounded-2xl" data-testid="skeleton-order-detail" />;
  if (!selected) return null;
  return <Card className="border-border bg-card" data-testid={`card-order-detail-${selected.id}`}><CardHeader className="flex-row items-start justify-between"><div><p className="eyebrow text-primary">Order record</p><CardTitle className="serif mt-2 text-2xl">#{selected.id.slice(0, 8)}</CardTitle></div><button onClick={onClose} className="rounded-lg p-1 text-muted-foreground hover:bg-muted" aria-label="Close order details" data-testid="button-close-order-detail"><X className="h-4 w-4" /></button></CardHeader><CardContent><StatusBadge status={selected.status} /><div className="mt-6 space-y-4 text-sm"><div className="rounded-xl border border-primary/20 bg-primary/5 p-4"><p className="flex items-center gap-2 text-xs font-medium uppercase tracking-[.12em] text-primary"><CalendarClock className="h-4 w-4" /> Pickup / delivery window</p><p className="mt-2 font-medium" data-testid={`text-detail-delivery-window-${selected.id}`}>{selected.deliveryWindow ?? 'Window pending'}</p></div><div className="flex justify-between"><span className="text-muted-foreground">Quantity</span><span className="font-medium" data-testid={`text-detail-order-quantity-${selected.id}`}>{selected.quantity}</span></div><div className="flex justify-between"><span className="text-muted-foreground">Unit price</span><span className="font-mono">{money(selected.unitPrice)}</span></div><div className="flex justify-between border-t border-border pt-4"><span className="font-medium">Total</span><span className="font-mono text-lg text-primary" data-testid={`text-detail-order-total-${selected.id}`}>{money(selected.totalAmount ?? 0)}</span></div>{selected.status === 'ready' ? <Button type="button" className="mt-2 h-11 w-full" onClick={onAdvance} disabled={updating} data-testid={`button-mark-delivered-${selected.id}`}>{updating ? <LoaderCircle className="animate-spin" /> : <Truck />} Mark picked up / delivered</Button> : null}</div></CardContent></Card>;
}
